    ...
    <div className="w-full flex flex-col justify-center">
      <div className="w-full max-w-6xl mx-auto">
        ...
        <motion.div
          ...
          className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-6 lg:gap-10 items-start text-left"
        >
          {/* LEFT COLUMN: Bio Text (~58% width on desktop) */}
          <div className="lg:col-span-7 flex flex-col gap-3.5 sm:gap-5 font-sans text-xs xs:text-base sm:text-[17px] text-[var(--text-body)] leading-[1.72]">
          ...
          <motion.div
            ...
            className="lg:col-span-5 w-full flex flex-col mt-1 sm:mt-2 lg:mt-0"
          >
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-panel)] p-3.5 xs:p-5 sm:p-7 md:p-8 shadow-[0_8px_30px_rgba(0,0,0,0.03)]">
              {/* Fact Sheet Header */}
              <div className="flex items-center justify-between pb-3 mb-3.5 sm:pb-3.5 sm:mb-4 border-b border-[var(--border)]">
                <span className="font-mono text-[11px] xs:text-xs font-semibold tracking-wider uppercase text-[var(--text-primary)]">
                  Fact Sheet
                </span>
                <span className="font-mono text-[10px] xs:text-[11px] text-[var(--text-muted)]">
                  [PROFILE_DATA]
                </span>
              </div>

              {/* Rows */}
              <ul className="space-y-3 sm:space-y-3.5 font-mono text-xs text-[var(--text-body)]">
                {factSheetItems.map((item, idx) => (
                  <li key={idx} className="flex items-start gap-2.5 sm:gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="font-medium text-[var(--text-primary)] mt-0.5 leading-snug break-words text-xs xs:text-[13px] sm:text-xs">
                        {item.value}
                      </div>
                    </div>
                  </li>
